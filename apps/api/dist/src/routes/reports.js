...
exports.reportsRouter.get("/", async (req, res) => {
    ...
    const revenue = sales.filter((sale) => sale.status === "APPROVED").reduce((sum, sale) => sum + (0, money_1.toNumber)(sale.total), 0);
    const credit = sales.filter((sale) => sale.paymentMethod === "CREDIT").reduce((sum, sale) => sum + (0, money_1.toNumber)(sale.total), 0);
    const products = new Map();
    for (const sale of sales) {
        for (const item of sale.items) {
            const row = products.get(item.productId) ?? { name: item.product.name, quantity: 0, total: 0 };
            row.quantity += item.quantity;
            row.total += (0, money_1.toNumber)(item.subtotal);
    ...
    res.json({
        ...
        totalSold: sales.reduce((sum, sale) => sum + (0, money_1.toNumber)(sale.total), 0),
