...
exports.dashboardRouter.get("/", async (_req, res) => {
    ...
    const [sales, products, customers] = await Promise.all([
        ...
        prisma_1.prisma.product.findMany({ orderBy: { name: "asc" } }),
        prisma_1.prisma.customer.findMany({ include: { sales: true, creditPayments: true } })
    ]);
    const approved = sales.filter((sale) => sale.status === "APPROVED");
    const revenue = approved.reduce((sum, sale) => sum + (0, money_1.toNumber)(sale.total), 0);
    const creditTotal = sales.filter((sale) => sale.paymentMethod === "CREDIT").reduce((sum, sale) => sum + (0, money_1.toNumber)(sale.total), 0);
    const received = approved.filter((sale) => sale.paymentMethod !== "CREDIT").reduce((sum, sale) => sum + (0, money_1.toNumber)(sale.total), 0);
    const lowStock = products.filter((product) => product.stockQuantity <= product.minimumStock);
    const soldItems = approved.flatMap((sale) => sale.items).reduce((acc, item) => {
        acc[item.product.name] = (acc[item.product.name] ?? 0) + item.quantity;
        return acc;
    }, {});
    const debtors = customers.filter((customer) => {
        const credit = customer.sales.filter((sale) => sale.paymentMethod === "CREDIT").reduce((sum, sale) => sum + (0, money_1.toNumber)(sale.total), 0);
    ...
    const byHour = Array.from({ length: 12 }, (_, index) => {
        ...
        return {
            ...
            faturamento: matching.reduce((sum, sale) => sum + (0, money_1.toNumber)(sale.total), 0)
        };
    });
    res.json({
        revenue,
