...
salesRouter.post("/", async (req, res) => {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const status: SaleStatus = isPix ? "PENDING" : "APPROVED";
    const pix = isPix ? await createPixPayment(total, "Venda Zamai Salgados") : null;

    const sale = await tx.sale.create({
      data: {
        customerId: body.customerId,
        subtotal: toDecimal(subtotal),
        discount: toDecimal(body.discount),
        total: toDecimal(total),
        paymentMethod: body.paymentMethod,
        status,
        creditStatus: isCredit ? "OPEN" : null,
        receivedAmount: body.receivedAmount == null ? null : toDecimal(body.receivedAmount),
        changeAmount: body.paymentMethod === "CASH" && body.receivedAmount ? toDecimal(Math.max(body.receivedAmount - total, 0)) : null,
...
salesRouter.post("/:id/approve-pix", async (req, res) => {
